...
internals.Request = function (options) {
    ...
    if (payload && !this.headers['content-length']){

        this.headers['content-length'] = (payload instanceof Buffer === true) ? payload.length.toString() : Buffer.byteLength(payload).toString();
    }
