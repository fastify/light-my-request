...
internals.Response = function (req, onEnd) {
    ...
    this.on('finish', internals.finish(this, req, onEnd));
    ...
    return function () {
        ...
        if (response.output &&
            ...
            for (var i = 0, il = response.output.length; i < il; ++i) {
                if (response.output[i] instanceof Buffer) {
                    output += response.output[i].toString();
                    raw.push(response.output[i]);
                }
                else {
                    var buffer = new Buffer(response.output[i], response.outputEncodings[i]);
        ...
        var rawBuffer = Buffer.concat(raw);
